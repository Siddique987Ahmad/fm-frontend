...
const ProductManagement: React.FC = () => {
  ...
  const handleAddProductSubmit = async (e: React.FormEvent) => {
    ...
    if (newProduct.allowedTransactions.length === 0) {
      setError('Please select at least one transaction type (Sale or Purchase)');
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-semibold text-gray-900">Product Catalog Management</h3>
        <button
      ...
      {showAddProductForm && (
        <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-gray-50">
          ...
          <form onSubmit={handleAddProductSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="pricePerUnit" className="block text-sm font-medium text-gray-700">Price per Unit (PKR)</label>
              <input 
                type="number" 
                name="pricePerUnit" 
                id="pricePerUnit" 
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Allowed Transactions</label>
              <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setNewProduct({
                          ...newProduct,
                          allowedTransactions: [...newProduct.allowedTransactions, 'sale']
                        });
                      } else {
                        setNewProduct({
                          ...newProduct,
                          allowedTransactions: newProduct.allowedTransactions.filter(t => t !== 'sale')
                        });
                      }
                    }}
                ...
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={newProduct.allowedTransactions.includes('purchase')}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setNewProduct({
                          ...newProduct,
                          allowedTransactions: [...newProduct.allowedTransactions, 'purchase']
                        });
                      } else {
                        setNewProduct({
                          ...newProduct,
                          allowedTransactions: newProduct.allowedTransactions.filter(t => t !== 'purchase')
                        });
                      }
                    }}
                    className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
              ...
              <p className="mt-1 text-xs text-gray-500">Select which transaction types are allowed for this product</p>
            </div>
      ...
      {editingProduct && (
        <div className="mb-8 p-6 border border-gray-200 rounded-lg bg-yellow-50">
          <h4 className="text-lg font-medium text-gray-800 mb-4">Edit Product: {editingProduct.name}</h4>
          <form onSubmit={handleUpdateProductSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="editPricePerUnit" className="block text-sm font-medium text-gray-700">Price per Unit (PKR)</label>
              <input 
                type="number" 
                name="pricePerUnit" 
                id="editPricePerUnit" 
            ...
            <div>
              ...
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={editingProduct.allowedTransactions.includes('sale')}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setEditingProduct({
                          ...editingProduct,
                          allowedTransactions: [...editingProduct.allowedTransactions, 'sale']
                        });
                      } else {
                        setEditingProduct({
                          ...editingProduct,
                          allowedTransactions: editingProduct.allowedTransactions.filter(t => t !== 'sale')
                        });
                      }
                    }}
                    className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                ...
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={editingProduct.allowedTransactions.includes('purchase')}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setEditingProduct({
                          ...editingProduct,
                          allowedTransactions: [...editingProduct.allowedTransactions, 'purchase']
                        });
                      } else {
                        setEditingProduct({
                          ...editingProduct,
                          allowedTransactions: editingProduct.allowedTransactions.filter(t => t !== 'purchase')
                        });
                      }
                    }}
                    className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Allowed Transactions</th>
