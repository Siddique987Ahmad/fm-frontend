...
const AdminDashboard: React.FC = () => {
  // State for edit modals and delete loading
  ...
  const handleDeleteSale = async (saleId: string) => {
    ...
    try {
      const { authenticatedFetch } = await import("../utils/apiClient");
      const result = await authenticatedFetch(`/admin/sales/${saleId}`, {
        method: "DELETE",
      });
      if (result.success) {
        fetchSales();
  ...
  // Placeholder for edit modals (implement UI as needed)

  const EditPurchaseModal = ({ purchase, onClose, onSave }: any) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white p-6 rounded shadow-lg w-full max-w-lg">
        <h2 className="text-lg font-semibold mb-4">Edit Purchase</h2>
        <pre className="mb-4 text-xs bg-gray-100 p-2 rounded">
  ...
  const EditSaleModal = ({ sale, onClose, onSave }: any) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white p-6 rounded shadow-lg w-full max-w-lg">
  ...
  // Edit Purchase Modal Save Handler
  const handleSaveEditPurchase = async (updatedPurchase: Purchase) => {
    try {
      const { authenticatedFetch } = await import("../utils/apiClient");
      const result = await authenticatedFetch(
        `/admin/purchases/${updatedPurchase._id}`,
        {
          method: "PUT",
  ...
  // Edit Sale Modal Save Handler
  const handleSaveEditSale = async (updatedSale: Sale) => {
    try {
      const { authenticatedFetch } = await import("../utils/apiClient");
      const result = await authenticatedFetch(
        `/admin/sales/${updatedSale._id}`,
        {
          method: "PUT",
  ...
  const [activeTab, setActiveTab] = useState<
    ...
    | "settings"
  >("dashboard");
  const [purchaseSubTab, setPurchaseSubTab] = useState<"create" | "list">(
    "create"
  );
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load users list
      try {
        const usersResult = await authenticatedFetch<{
          success: boolean;
          data?: any;
  ...
  const fetchProductTypes = async () => {
    try {
      const { fetchProductTypes: fetchTypes } = await import(
        "../utils/productTypes"
      );
  ...
  const handlePurchaseFormChange = (field: string, value: string) => {
    ...
    if (field === "amountPaid" || field === "totalAmount") {
      const amountPaid = parseFloat(
        field === "amountPaid" ? value : updated.amountPaid
      );
      const totalAmount = parseFloat(
        field === "totalAmount" ? value : updated.totalAmount
      );
  ...
  const handleSalesFormChange = (field: string, value: string) => {
    ...
    if (field === "amountReceived" || field === "totalAmount") {
      const amountReceived = parseFloat(
        field === "amountReceived" ? value : updated.amountReceived
      );
      const totalAmount = parseFloat(
        field === "totalAmount" ? value : updated.totalAmount
      );
  ...
  const handleCreatePurchase = async () => {
    try {
      ...
      const result = await authenticatedFetch<{ success: boolean; data?: any }>(
        `/products/${purchaseForm.productType
          .toLowerCase()
          .replace(/\s+/g, "-")}`,
        {
          method: "POST",
          body: JSON.stringify({
            ...
            remainingAmount:
              ...
              parseFloat(purchaseForm.amountPaid),
  ...
  const handleCreateSales = async () => {
    try {
      ...
      const { authenticatedFetch } = await import("../utils/apiClient");
      const result = await authenticatedFetch<{ success: boolean; data?: any }>(
        `/products/${salesForm.productType.toLowerCase().replace(/\s+/g, "-")}`,
        {
          method: "POST",
          body: JSON.stringify({
            ...
            remainingAmount:
              parseFloat(salesForm.totalAmount) -
              parseFloat(salesForm.amountReceived),
    ...
    try {
      ...
      if (productTypes.length > 0) {
        ...
        for (const productType of productTypes) {
          console.log(
            "Fetching transactions for product type:",
            productType.name
          );
          try {
            const transactionsResult = await authenticatedFetch<{
              success: boolean;
              data?: TransactionData[];
            }>(`/products/${productType.value}`);
            console.log(
              "Transactions result for",
              productType.name,
              ":",
              transactionsResult
            );
            if (transactionsResult.success && transactionsResult.data) {
              const purchases = transactionsResult.data
                .filter(
                  (t: TransactionData) => t.transactionType === "purchase"
                )
            ...
            console.error(
              `Error fetching transactions for ${productType.name}:`,
              err
            );
          }
        }
  ...
  return (
    <>
      {/* Modals are rendered at the end of the page to ensure they overlay all content */}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-white text-opacity-80">
                          Here's what's happening with your factory management
                          system today.
                        </p>
                      </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handlePurchaseFormChange("rate", e.target.value)
                              }
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleSalesFormChange("rate", e.target.value)
                              }
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                              placeholder="Enter rate per unit"
                            />
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                handleSalesFormChange("notes", e.target.value)
                              }
                              rows={3}
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
                              placeholder="Enter any additional notes"
      ...
      {/* Edit Purchase Modal */}
      {showEditPurchaseModal && editPurchase && (
        <EditPurchaseModal
